...
<div class="card-body pt-0">
    <form method="post" action="<?= URL ?>ajax/distributor.add.ajax.php">
        ...
        <div class="col-md-12">
            <label class="mb-0 mt-1" for="distributor-gstid">GST ID</label>
            <input type="text" class="form-control" id="distributor-gstid" name="distributor-gstid" placeholder="Distributor GST ID" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="15" oninput="javascript: if (this.value.length > this.minLength) this.value = this.value.slice(0, this.minLength);" minlength="18" required>
        </div>
