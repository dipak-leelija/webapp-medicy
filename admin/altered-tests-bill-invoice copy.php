...
if ($updateVisit) {
    ...
    $updateBill = $LabBilling->updateLabBill($billId, $billingDate, $referedDoc, $testDate,  $totalAmount, $discountOnTotal, $totalAfterDiscount, $cgst, $sgst, $paidAmount, $dueAmount, $status);
    ...
    /* ========================= Bill Details Insertion End ========================= */
        ...
        // $exists = count($billDetails);
        // $id = $testIds;
        // $totalId = count($id);
        // if ($exists != NULL || $exists != 0) {
        //     for ($i=0; $i < $exists; $i++) { 
        //         // echo $exists.'<br>';
        //         foreach ($id as $testId) {
        //             $testId                 = array_shift($id);
        //             $testPrice              = array_shift($priceOfTestBck);
        //             $percentageOfDiscount   = array_shift($testDiscsBck);
        //             $priceAfterDiscount     = array_shift($testAmountsBck);
