<?php
    ...
    $distName = $Distributor->distributorDetail($maxPurchase->distributor_id);
    // $distName = json_encode($distName);
...
<script>
    const checkSell = (id) => {
        ...
        if (id == 'amount') {
            maxAmountUrl = `../admin/ajax/dist-on-sales.ajax.php?amount=${id}`;
            xmlhttp.open("GET", maxAmountUrl, false);
            xmlhttp.send(null);
